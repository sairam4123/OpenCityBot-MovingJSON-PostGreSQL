...
async def message_history_into_transcript_file_object(channel: discord.TextChannel, member: discord.Member, id_: int) -> discord.File:
    ...
    try:
        ...
        pass
    transcript_temp = f"Transcript for {member.name}#{member.discriminator} ({member.id}) \n"
    file1 = BytesIO(initial_bytes=bytes(transcript_temp + "\n".join(
        f"{convert_utc_into_ist(transcript.created_at)[1]}   {transcript.author.name}#{transcript.author.discriminator} ({transcript.author.id}): {transcript.content}" for
